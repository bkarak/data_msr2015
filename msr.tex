...
In this paper, we present a dataset that contains popular metrics, focusing on providing calculation for a large collection of the Java ecosystem. For this purpose we selected the maven~\cite{MAVEN} repository. The tools that were used are the \textit{ckjm} \cite{Spi05g}, \textit{jdepend}~\cite{JDEPEND}, and \textit{clmt}~\cite{SGKL09}. These tools focus on three main aspects of a software system; object-oriented design, program size, and package design.

The rest of the paper is structured as follows; first, the datasert construction process is presented and analysed. Then we analyse the schema of the database. Along with the dataset limitations, we present examples and ideas of basic experimentation with the measurements.
