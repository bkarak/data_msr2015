...
\begin{itemize}
  ...
  \item \textbf{JDepend} \textit{(jdep)}~\cite{JDEPEND} is an source tool that analyses {\sc jar} files that contain compiled Java classes and calculates a series of design metrics.
...
Table~\ref{tbl:selected-metrics} presents the key metrics that are calculated and stored in the dataset. Each tool focuses on a different aspect of a software system; \textit{ckjm} focuses on object-oriented design metrics, \textit{jdepend} on
...
Figure~\ref{fig:database-schema} illustrates the database
